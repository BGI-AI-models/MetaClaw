"""
reference_enrichment-analysis.py — TEMPLATE (read-only at /pipeline/scripts/ inside the
downstream container). DO NOT run this directly for real analyses: copy it to
/job/reproducibility/generated_scripts/enrichment-analysis_<YYYYMMDD-HHMMSS>.py, customize
for your dataset, then invoke:

    bash gateway/run_downstream.sh <job-id> --skills enrichment-analysis

Pathway enrichment on HUMAnN3 pathabundance products.

Contract:
    --job-dir <path>   (default: /job)
    Reads    : <job-dir>/stage/
    Writes   : <job-dir>/analysis/enrichment/
    Meta     : <job-dir>/analysis/enrichment/enrichment-analysis_meta.json
"""
...
ANALYSIS_SUBDIR = "enrichment"
STAGE_SUBDIR = ""  # may be None
