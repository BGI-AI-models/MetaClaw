"""
reference_clustering.py — TEMPLATE (read-only at /pipeline/scripts/ inside the
downstream container). DO NOT run this directly for real analyses: copy it to
/job/reproducibility/generated_scripts/clustering_<YYYYMMDD-HHMMSS>.py, customize
for your dataset, then invoke:

    bash gateway/run_downstream.sh <job-id> --skills clustering

Unsupervised clustering on a staged tabular dataset.

Contract:
    --job-dir <path>   (default: /job)
    Reads    : <job-dir>/stage/clustering
    Writes   : <job-dir>/analysis/clustering/
    Meta     : <job-dir>/analysis/clustering/clustering_meta.json
"""
...
SKILL = "clustering"
ANALYSIS_SUBDIR = "clustering"
STAGE_SUBDIR = "clustering"  # may be None
