"""
reference_classification-pipeline.py — TEMPLATE (read-only at /pipeline/scripts/ inside the
downstream container). DO NOT run this directly for real analyses: copy it to
/job/reproducibility/generated_scripts/classification-pipeline_<YYYYMMDD-HHMMSS>.py, customize
for your dataset, then invoke:

    bash gateway/run_downstream.sh <job-id> --skills classification-pipeline

Supervised classification on a staged tabular dataset.

Contract:
    --job-dir <path>   (default: /job)
    Reads    : <job-dir>/stage/classification
    Writes   : <job-dir>/analysis/classification-pipeline/
    Meta     : <job-dir>/analysis/classification-pipeline/classification-pipeline_meta.json
"""
...
ANALYSIS_SUBDIR = "classification-pipeline"
STAGE_SUBDIR = "classification"  # may be None
